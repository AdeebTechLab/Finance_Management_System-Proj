...
export default function InvestmentTracker() {
  ...
  // on mount: load & sort existing investments from localStorage
  useEffect(() => {
    const raw = localStorage.getItem("userHistory")
    if (!raw) return console.warn("No userHistory in localStorage")

    try {
      const parsed = JSON.parse(raw)
      const loaded = parsed.investments || []
      setInvestments(sortByDateDesc(loaded))
    } catch (error) {
  ...
  const addInvestment = async () => {
    if (!company || !type || !amount || !returns) {
    ...
    const newInvestment = {
      ...
      date: new Date().toLocaleDateString(), // if you want ISO for safer sorting: .toISOString()
    }
    ...
    try {
      ...
      if (!response.ok) {
        return alert(responseData.message || "Failed to add Investment")
      }

      // merge server response (to get any server-generated fields) with our local data
      const saved = { ...newInvestment, ...responseData }
      // prepend & re-sort just in case
      const updatedList = sortByDateDesc([saved, ...investments])

      // update React state
      setInvestments(updatedList)

      // persist back into userHistory
      const rawHistory = localStorage.getItem("userHistory")
      if (rawHistory) {
        const history = JSON.parse(rawHistory)
        history.investments = updatedList
  ...
  // download PDF of the investments list
    ...
    <div className="max-w-[1200px] mx-auto min-h-screen px-4 md:px-6 lg:px-8 py-8 flex flex-col gap-8">
      <div className="p-6 max-w-4xl mx-auto bg-gradient-to-b from-gray-50 to-white rounded-2xl shadow-xl">
        ...
        <div className="p-6 bg-white border border-indigo-100 rounded-xl shadow-lg space-y-5 mb-8 transform transition-all hover:shadow-xl">
          <h2 className="text-xl font-semibold text-indigo-700 border-b border-indigo-100 pb-3">
            Add New Investment
          </h2>
